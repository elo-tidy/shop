...
import type { Transporter, DeliveryMode } from '@/types/ShippingMode'
...
export const useOrderStore = defineStore(
  ...
  () => {
    ...
    const deliveryDetails = ref<{deliveryMode: DeliveryMode['name'], deliveryDate: String | null,  transporter:Transporter} | null>(null)
    ...
    const totalPriceInCents = computed(() => priceFromEurosToCents(totalPrice.value))
    ...
    const stripePayload = computed(() => orderModel.value?.stripePayload ?? null)
    ...
    function initOrder(userId: string, paymentIntentId: string | null = null ) {
        ...
        const orderDraft: Order = {
            ...
            total_price: Number(numberWithTwoDecimals(productsPrice + deliveryPrice)),
